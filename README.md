...
En servidor, GitHub CLI ha d'estar instal·lat i autenticat. Si el servei no troba `gh`, configura `GH_BIN=/ruta/al/gh` en `.env`. Si vols usar un token específic per a `gh`, posa'l en `GH_TOKEN`; `GITHUB_TOKEN` queda reservat per a les crides del dashboard a l'API de GitHub. Per crear repositoris privats, convidar alumnat i esborrar repositoris, eixe `GH_TOKEN` ha de poder administrar repositoris en l'organització docent; amb tokens clàssics sol caldre `repo` i `delete_repo`.
